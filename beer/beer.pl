...
category(jasne_lagery) :- barwa(jasna), lager.
category(pilsnery) :- aromat(korzenny), smak(gorzki), barwa(zlota), gorycz(niskasrednia).
category(europejskie_bursztynowe_lagery) :- barwa(bursztynowa), lager.
category(ciemne_lagery) :- barwa(brazowa), lager.
category(kozlaki) :- kozlak.
category(jasne_hybrydowe) :- barwa(zlota), gorycz(niskasrednia).
category(bursztynowe_hybrydowe) :- barwa(czerwona), moc(4.8).
category(english_pale_ale) :- barwa(bursztynowa), gorycz(niskasrednia), bitter.
category(portery) :- barwa(czarna), gorycz(niskasrednia).
category(american_ale) :- gorycz(niskasrednia), american_ale.
category(niemieckie_pszeniczne_zytnie) :- aromat(zbozowy), gorycz(niska).
category(wedzone) :- aromat(wedzone), gorycz(niska).
category(belgijskie_mocne_ale) :- gorycz(niskasrednia).
category(mocne_ale) :- gorycz(sredniawysoka), barwa(czerwona).
category(kwasne_ale) :- gorycz(niska), sour.
category(belgijskie_i_francuskie_ale) :- gorycz(niska).
category(stouty) :- barwa(czarna), stout.
category(english_brown_ale) :- gorycz(niska).
category(india_pale_ale) :- barwa(bursztynowa), smak(gorzki), gorycz(wysoka).
...
beerstyle(kremowe_ale) :- aromat(slodowy), smak(slodki), category(jasne_hybrydowe).
...
beerstyle(szkockie_lekkie_60) :- category(scottish_irish_ale), barwa(czerwona), scottish.
beerstyle(szkockie_mocne_70) :- category(scottish_irish_ale), barwa(czerwona), scottish.
beerstyle(szkockie_eksportowe_80) :- category(scottish_irish_ale), barwa(czerwona), scottish.
beerstyle(irlandzkie_czerwone_ale) :- aromat(maslany), smak(slodki), category(scottish_irish_ale), barwa(czerwona), moc(5.0).
beerstyle(szkockie_mocne_ale) :- category(scottish_irish_ale), barwa(brazowa), scottish.
...
beerstyle(berlinskie_pszeniczne) :- category(kwasne_ale), barwa(jasna).
beerstyle(flamandzkie_czerwone_ale) :- category(kwasne_ale), barwa(czerwona).
beerstyle(flamandzkie_brazowe_ale) :- category(kwasne_ale), barwa(brazowa).
beerstyle(lambik) :- aromat(miodowy), category(kwasne_ale), barwa(zlota).
beerstyle(gueuze) :- aromat(cytrusowy), category(kwasne_ale), barwa(zlota).
beerstyle(owocowy_lambik) :- category(kwasne_ale), barwa(zlota).
...
beerstyle(belgian_golden_strong_ale) :- aromat(kwiatowy), smak(gorzki), category(belgijskie_mocne_ale), barwa(zlota).
